...
def set_slice(*args: object) -> None:
    if len(args) != 7:
        raise ValueError(
            "/slice/set expects request_index object_index samples nx ny nz distance"
        )
    request_index = as_int(args[0])
    object_index = as_int(args[1])
    sample_count = as_int(args[2])
    obj = object_by_index(object_index)
    normal = normal_vector(args[3:6])
    distance = as_float(args[6])
    bvh = bvh_for_object(obj)
    radii = sample_radii(bvh, normal, distance, sample_count)
    client.send_message(b"/slice/radii", [request_index, object_index, *radii])
